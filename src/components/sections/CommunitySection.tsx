...
export function CommunitySection({ community }) {
  ...
  const handleNewsletterSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (!isAllowedEmail(v)) {
        setNewsletterError(
          isValidEmailFormat(v) ? DISALLOWED_EMAIL_MESSAGE : INVALID_EMAIL_MESSAGE,
        );
        setNewsletterSuccess(null);
        return;
      }
  ...
  return (
    <section id="community-section" className="site-section">
      <div className="container-standard section-inner">
        ...
        <div className="grid gap-8 xl:grid-cols-12 xl:gap-[32px]">
          ...
          <ResponsiveMotionDiv
            ...
            className={`relative z-10 flex min-h-0 w-full flex-1 flex-col items-center overflow-hidden border border-white/10 backdrop-blur-xl md:items-start md:justify-between xl:col-span-8 ${cardBase}`}
            style={{
              background:
          ...
          >
            ...
            <div className="pointer-events-none absolute -right-10 -top-10 opacity-[0.03] hidden sm:block">
              <svg
                className="h-64 w-64 sm:v-hidden"
                viewBox="0 0 24 24"
                fill="white"
                aria-hidden
              >
          ...
          <ResponsiveMotionDiv
            ...
            className={`flex min-h-0 w-full flex-1 flex-col items-center rounded-[16px] bg-black md:items-stretch md:justify-between xl:col-span-4 ${cardBase}`}
          >
            <CardIconTitle
            ...
            <ResponsiveMotionA
              ...
              target="_blank"
              rel="noopener noreferrer"
              motionProps={{ whileTap: { opacity: 0.92 } }}
              className="mx-auto my-6 flex h-14 w-full max-w-xs items-center justify-center rounded-[16px] bg-white text-sm font-bold text-black transition-colors hover:bg-zinc-200 md:mx-0 md:my-auto md:max-w-none"
            >
          ...
          <ResponsiveMotionDiv
            ...
            className={`flex min-h-0 w-full flex-1 flex-col items-center md:items-stretch md:justify-between xl:col-span-4 ${cardBase}`}
            style={{
              background:
                "linear-gradient(0deg, #1b3144, #1b3144), radial-gradient(106.03% 300.79% at 100% 0%, rgba(36, 186, 206, 0.12) 0%, rgba(36, 186, 206, 0) 60%)",
            }}
          >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-12 inline-flex items-center justify-center gap-2 text-sm font-bold text-Elementa-accent transition-opacity hover:opacity-90 md:mt-auto md:justify-start md:pt-6"
            >
          ...
          >
            <picture className="pointer-events-none absolute inset-0 block size-full overflow-hidden">
              <source
                media="(min-width: 768px)"
                srcSet="/assets/social-card-bg.webp"
              />
              <img
                src="/assets/social-card-bg-mobile.webp"
                alt=""
                aria-hidden
            ...
            <div className="relative z-10 flex w-full max-w-lg flex-col items-center md:max-w-none md:items-start">
              ...
              <form
                className="mt-10 flex w-full max-w-md flex-col items-center sm:max-w-lg md:max-w-lg md:items-stretch"
                onSubmit={handleNewsletterSubmit}
                noValidate
              >
                <div className="flex w-full flex-col items-stretch gap-3 sm:flex-row sm:items-stretch sm:gap-2">
                  <input
                    type="email"
                    ...
                    className={`min-h-[52px] min-w-0 flex-1 rounded-[16px] border border-white/35 bg-white/[0.08] px-4 py-3 text-center text-sm text-white shadow-[inset_0_1px_0_0_rgba(255,255,255,0.07)] outline-none placeholder:text-zinc-400/90 transition-[border-color,box-shadow] duration-200 focus-visible:ring-2 focus-visible:ring-[#24bace]/45 max-md:backdrop-blur-none sm:text-left md:text-left md:backdrop-blur-xl ${
                      newsletterError
