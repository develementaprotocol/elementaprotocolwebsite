...
/** Desktop/tablet carousel — md+ only (dynamically imported). */
export function SocialProofDesktop({ socialProof }: { socialProof: SocialProof }) {
  ...
  const indexRef = useRef(socialProof.testimonials.length);
  ...
  const contentPaddingRef = useRef(0);
  ...
  const applyTrackPosition = useCallback(
    (offset = 0) => {
      ...
      const xPos = contentPaddingRef.current - indexRef.current * stepRef.current + offset;
  ...
  useLayoutEffect(() => {
    ...
    const track = trackRef.current;
    if (!track || viewportW <= 0) return;
    const xPos = contentPadding - index * stepRef.current + dragOffset;
    track.style.transition =
      isJumping || reduced
        ? "none"
        : "transform 380ms cubic-bezier(0.25, 0.1, 0.25, 1)";
    track.style.transform = `translateX(${xPos}px)`;
  }, [index, dragOffset, isDragging, isJumping, reduced, viewportW, contentPadding, step]);

  const finishDrag = useCallback(
    (clientX: number) => {
      if (!isDraggingRef.current) return;

      const swipe = clientX - dragStartX.current;
      const currentStep = stepRef.current;
      const swipedSteps = currentStep > 0 ? Math.round(-swipe / currentStep) : 0;

      if (Math.abs(swipedSteps) > 0) {
        setIndex(wrapIndex(dragStartIndex.current + swipedSteps));
      } else if (swipe < -40) {
        setIndex(wrapIndex(dragStartIndex.current + 1));
      } else if (swipe > 40) {
        setIndex(wrapIndex(dragStartIndex.current - 1));
      }
  ...
  const onCarouselPointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      const onMove = (ev: PointerEvent) => {
        if (!isDraggingRef.current) return;
        const offset = ev.clientX - dragStartX.current;
        dragOffsetRef.current = offset;
        applyTrackPosition(offset);
      };
    ...
    >
          ...
          {[...marqueePartners, ...marqueePartners].map((p, i) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                />
                <span className="rounded  px-2 py-0.5 font-display text-sm font-bold uppercase tracking-widest text-[#081421] sm:text-base">
                  {p.name}
                </span>
              </div>
            </div>
      ...
      <div className="relative z-10 w-full px-0">
        <div className="pb-12 pt-12">
          <div
            ref={viewportRef}
            className={`w-full min-w-0 overflow-hidden min-h-[min(280px,50vh)] select-none ${
